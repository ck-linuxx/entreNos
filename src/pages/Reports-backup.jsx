...
export default function Reports() {
  ...
  // Calcular despesas pagas pelo usuário logado vs Ana
  const userExpenses = data.expenses.filter(exp => exp.paidBy === getUserName()).reduce((acc, exp) => acc + exp.amount, 0);
  const anaExpenses = data.expenses.filter(exp => exp.paidBy === 'Ana').reduce((acc, exp) => acc + exp.amount, 0);
  const totalExpenses = userExpenses + anaExpenses;
    ...
    <div className="max-w-7xl mx-auto p-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
        ...
        <div className="card bg-gradient-to-br from-green-500 to-green-600 text-white">
          <div className="text-center">
            <p className="text-green-100 text-sm font-medium mb-2">{getUserName()} Pagou</p>
            <p className="text-2xl font-bold">
              R$ {userExpenses.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}
            </p>
            <p className="text-green-100 text-sm">({userPercentage.toFixed(0)}%)</p>
          </div>
        </div>
              ...
              <div className={`text-center p-4 rounded-lg ${balance.balance > 0
                ...
                {balance.balance > 0 ? (
                  <p className="text-lg font-bold text-green-600 dark:text-green-400">
                    Ana deve R$ {Math.abs(balance.balance).toLocaleString('pt-BR', { minimumFractionDigits: 2 })} para {getUserName()}
                  </p>
                ) : balance.balance < 0 ? (
                  <p className="text-lg font-bold text-red-600 dark:text-red-400">
                    {getUserName()} deve R$ {Math.abs(balance.balance).toLocaleString('pt-BR', { minimumFractionDigits: 2 })} para Ana
                  </p>
                ) : (
